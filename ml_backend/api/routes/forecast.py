...
from models.forecast import get_forecaster, EnrollmentForecaster
...
class StateForecastResponse(BaseModel):
    """Response for bulk state forecasts."""
    model_version: str
    trained_at: str
    periods: int
    confidence_level: float
...
class StateTrainResponse(BaseModel):
    """Response for state model training."""
    status: str
    model_version: str
    trained_count: int
    failed_count: int
    trained_states: List[str]
    metrics: dict
...
async def get_available_states():
    """
    Get list of states with trained forecast models.
    """
    forecaster = get_forecaster()
...
@router.get("/predict-all-states", response_model=StateForecastResponse)
async def predict_all_states(
    periods: int = Query(6, ge=1, le=24, description="Forecast horizon"),
    confidence: float = Query(0.95, ge=0.5, le=0.99, description="Confidence level")
):
    """
    Get bulk enrollment forecasts for all Indian states in a single response.
    
    Returns model version, backtest metrics (MAPE, MAE, RMSE), and forecasts
    with confidence intervals for each state.
    """
    forecaster = get_forecaster()
    
    if not forecaster.get_available_states():
        raise HTTPException(
            status_code=400,
            detail="No state models trained. Call POST /api/forecast/train/states first."
        )
    
    result = forecaster.forecast_all_states(periods=periods, confidence_level=confidence)
    
    return StateForecastResponse(
        model_version=result.get("model_version", "unknown"),
        trained_at=result.get("trained_at", ""),
        periods=result.get("periods", periods),
        confidence_level=result.get("confidence_level", confidence),
        total_states=result.get("total_states", 0),
        states=result.get("states", [])
    )


@router.post("/train/states", response_model=StateTrainResponse)
async def train_state_forecast_models(
    limit: int = Query(1000, ge=100, le=10000, description="Records to fetch for training")
):
    """
    Train ARIMA models for state-level enrollment forecasting.
    
    Aggregates district data to state level, trains one model per state,
    computes backtest metrics (MAPE, MAE, RMSE), and versions the model.
    
    Returns training summary with model version and metrics.
    """
    import pandas as pd
    import numpy as np
    from datetime import datetime, timedelta
    ...
    try:
        logger.info(f"Starting state-level model training with limit={limit}")
        
        # Get forecaster instance first
        forecaster = get_forecaster()
        ...
        if not state_series:
            raise HTTPException(
                status_code=400,
                detail="No states with sufficient data for training"
            )
        
        # Train state models
        results = forecaster.train_state_models(state_series, order=(1, 1, 1))
        
        # Save models
        forecaster.save_model()
        
        return StateTrainResponse(
            status="success",
            model_version=results.get("model_version", "unknown"),
            trained_count=results["total_trained"],
            failed_count=results["total_failed"],
            trained_states=results["trained"],
        ...
        logger.error(f"State training failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/predict/state/{state}")
async def predict_single_state(
    state: str,
    periods: int = Query(6, ge=1, le=24, description="Forecast horizon"),
    confidence: float = Query(0.95, ge=0.5, le=0.99, description="Confidence level")
):
    ...
    result = forecaster._forecast_entity(model_key, periods=periods, confidence_level=confidence)
